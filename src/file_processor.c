...
#define BUFFER_LENGTH                                                          \
  (1024 * (EVENT_SIZE + 16)) // Tamaño del buffer para eventos
...
int main() {
  ...
  {
    processFilesProcess();
  } else if (proceso_patrones ==
             0) // Proceso hijo -> Proceso de comprobar patrones
  {
...
int checkPatternsProcess() {
  ...
  if (pthread_mutex_init(&mutexPatterns, NULL) != 0) {
    printLogScreen(mutexLogFile, config_file.log_file, PATTERN_MUTEX_ERROR,
                   PATTERN_MUTEX_ERROR);
    return -1;
  }

  while (1) {
    th_pattern1 = pthread_create(&th_pattern1, NULL, pattern1,
                                 config_file.inventory_file);
    th_pattern2 = pthread_create(&th_pattern2, NULL, pattern2,
                                 config_file.inventory_file);
    th_pattern3 = pthread_create(&th_pattern3, NULL, pattern3,
                                 config_file.inventory_file);
    th_pattern4 = pthread_create(&th_pattern4, NULL, pattern4,
                                 config_file.inventory_file);
    th_pattern5 = pthread_create(&th_pattern5, NULL, pattern5,
                                 config_file.inventory_file);
