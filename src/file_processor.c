...
/// @param folder_struct Directorio del que queremos verificar la llegada de
/// nuevos archivos
...
int CreateSharedMemory(size_t size, int *idSharedMemory,
...
/// @param sharedMemory_ptr Puntero a la memoria compartida
/// @return 0 si éxito, -1 error
int ResizeSharedMemory(int *idSharedMemory, size_t newSize,
                       shared_memory **sharedMemory_ptr);
...
void AddDataSharedMemory(int *idSharedMemory, shared_memory **sharedMemory_ptr,
                         sucursal_info sucInfo);
...
void ConsolidateMemory(shared_memory *sharedMemory_ptr,
                       const char *ConsolidatedPath);
...
int main() {
  ...
  if (CreateSharedMemory(config_file.size_fp, &IDSharedMemory,
                         &SharedMemory_ptr) == -1) {
...
int ResizeSharedMemory(int *idSharedMemory, size_t newSize,
  ...
  if (newSharedMemoryID == -1) {
    printf(
        "Error al crear la zona de memoria compartida ResizeSharedMemory.\n");
    return -1;
  }

  // Asociar la nueva memoria compartida
  shared_memory *newMemory_ptr =
      (shared_memory *)shmat(newSharedMemoryID, NULL, 0);
  ...
  size_t dataSize =
      prevUsedSize - sizeof(shared_memory) + sizeof(sucursal_info) * filesCount;
...
void AddDataSharedMemory(int *idSharedMemory, shared_memory **sharedMemory_ptr,
                         sucursal_info sucInfo) {
  if ((*sharedMemory_ptr)->usedSize + sizeof(sucInfo) >
      (*sharedMemory_ptr)->mcSize) {
...
void ConsolidateMemory(shared_memory *sharedMemory_ptr,
  ...
  for (size_t i = 0; i < sharedMemory_ptr->filesCount; i++) {
    ...
    fprintf(consolidated_ptr, "%c;%s;%d\n", sucInfo.sucursal_number,
            sucInfo.line, sucInfo.flag);
  }
...
sucursal_file *newFile(char *file_path, char *file_name, char sucursal_number) {
  ...
  pthread_cond_signal(&cond); // Avisamos a los hilos que deben comprobar si hay
                              // un nuevo archivo
...
void processFiles(sucursal_file *file, shared_memory *sharedMemory_ptr) {
  ...
  char newDataPath[100] =
      "../files_data/processed/"; // Ruta de la carpeta de ficheros procesados

  FILE *sucursal_file =
      fopen(file->file_path, "r"); // Archivo sucursal a procesar
  ...
  // Bucle que leerá el fichero de la sucursal hasta que no haya más información
  // y la escribirá en el fichero consolidado

  // ****A METER A FICHERO CONSOLIDADO. COMENTADO PARA METERLO EN MC
  /*while (fgets(line, sizeof(line), sucursal_file))
  {
      line[strcspn(line, "\n")] = '\0'; // Elimina el salto de línea
      fprintf(consolidated_file, "%c;%s;%d\n", file->sucursal_number, line,
  flag); // Escribir en el fichero consolidado file->num_operations++; //
  Incrementar el número de operaciones
  }*/
...
void *verifyNewFile(void *folder_struct) {
  ...
  watchDescriptor = inotify_add_watch(
      fileDescriptor, ((sucursal_dir *)folder_struct)->folder_name, IN_CREATE);
  ...
  while (1) {
    ...
    while (i < length) {
          ...
          IN_CREATE) // Se comprueba si se ha creado un nuevo archivo
      {
        ...
        closedir(
            ((sucursal_dir *)folder_struct)->folder); // Cerrar el directorio
        ((sucursal_dir *)folder_struct)->folder =
            opendir(((sucursal_dir *)folder_struct)
                        ->folder_name); // Abrir el directorio de nuevo
      }
...
int processFilesProcess() {
  ...
  sucursal_dir *suc_1_dir = (sucursal_dir *)malloc(
      sizeof(sucursal_dir)); // Reservamos memoria para la sucursal 1
  sucursal_dir *suc_2_dir = (sucursal_dir *)malloc(
      sizeof(sucursal_dir)); // Reservamos memoria para la sucursal 2
  sucursal_dir *suc_3_dir = (sucursal_dir *)malloc(
      sizeof(sucursal_dir)); // Reservamos memoria para la sucursal 3
  sucursal_dir *suc_4_dir = (sucursal_dir *)malloc(
      sizeof(sucursal_dir)); // Reservamos memoria para la sucursal 4

  sprintf(suc_1_dir_name, "%s/%s1/", config_file.path_files,
          config_file.suc_dir); // Ruta del directorio de la sucursal 1
  sprintf(suc_2_dir_name, "%s/%s2/", config_file.path_files,
          config_file.suc_dir); // Ruta del directorio de la sucursal 1
  sprintf(suc_3_dir_name, "%s/%s3/", config_file.path_files,
          config_file.suc_dir); // Ruta del directorio de la sucursal 1
  sprintf(suc_4_dir_name, "%s/%s4/", config_file.path_files,
          config_file.suc_dir); // Ruta del directorio de la sucursal 1

  strcpy(suc_1_dir->folder_name,
         suc_1_dir_name); // Almacenamos en la estructura del directorio de la
                          // sucursal 1 la ruta
  strcpy(suc_2_dir->folder_name,
         suc_2_dir_name); // Almacenamos en la estructura del directorio de la
                          // sucursal 1 la ruta
  strcpy(suc_3_dir->folder_name,
         suc_3_dir_name); // Almacenamos en la estructura del directorio de la
                          // sucursal 1 la ruta
  strcpy(suc_4_dir->folder_name,
         suc_4_dir_name); // Almacenamos en la estructura del directorio de la
                          // sucursal 1 la ruta
  ...
  pthread_t newFileThread_SUC1, newFileThread_SUC2, newFileThread_SUC3,
      newFileThread_SUC4;

  int controler_SUC1, controler_SUC2, controler_SUC3, controler_SUC4;

  controler_SUC1 =
      pthread_create(&newFileThread_SUC1, NULL, verifyNewFile, suc_1_dir);
  controler_SUC2 =
      pthread_create(&newFileThread_SUC2, NULL, verifyNewFile, suc_2_dir);
  controler_SUC3 =
      pthread_create(&newFileThread_SUC3, NULL, verifyNewFile, suc_3_dir);
  controler_SUC4 =
      pthread_create(&newFileThread_SUC4, NULL, verifyNewFile, suc_4_dir);

  if (controler_SUC1 < 0 || controler_SUC2 < 0 || controler_SUC3 < 0 ||
      controler_SUC4 < 0) {
  ...
  pthread_create(&th1, NULL, processSucursalDirectory,
                 suc_1_dir); // Crear hilo sucursal 1
  pthread_create(&th2, NULL, processSucursalDirectory,
                 suc_2_dir); // Crear hilo sucursal 2
  pthread_create(&th3, NULL, processSucursalDirectory,
                 suc_3_dir); // Crear hilo sucursal 3
  pthread_create(&th4, NULL, processSucursalDirectory,
                 suc_4_dir); // Crear hilo sucursal 4
...
void *processSucursalDirectory(void *folder_struct) {
  ...
  ((sucursal_dir *)folder_struct)->folder =
      opendir(((sucursal_dir *)folder_struct)->folder_name);

  if (((sucursal_dir *)folder_struct)->folder ==
      NULL) // Error al abrir el directorio
  {
    sprintf(logString, FOLDER_OPEN_ERROR,
            ((sucursal_dir *)folder_struct)->folder);
    printLogScreen(mutexLogFile, config_file.log_file, logString, logString);
    free(logString);
    free(screenString);
  ...
  while (1) {
    while ((directorio = readdir(((sucursal_dir *)folder_struct)->folder)) !=
           NULL) // Leer el directorio
    {

      if (directorio->d_type == DT_REG) // Comprobar que sea un archivo
      {
        sprintf(filePath, "%s%s", ((sucursal_dir *)folder_struct)->folder_name,
                directorio->d_name); // Ruta al archivo
        nueva_sucursal = newFile(
            filePath, directorio->d_name,
            filePath[19]); // Añadimos un archivo de la sucursal 1 a la lista
        sem_wait(&sem_thread_creation);
        pthread_create(
            &th, NULL, reader,
            nueva_sucursal); // Creamos el hilo para procesar la sucursal
...
void *pattern1(void *arg) {
  sem_init(&mutex1, 0, 1);
  sem_wait(&mutex1);
  // Ordeno el vector de "registros" por usuario y fecha a la vez, para pasarle
  // el filtro del patron 1.
  if (num_registros > 1) {
  ...
  // En Usuarios guardaré los registros que cumplen el patron. (Misma Hora,
  // Mismo Usuario)
  struct Operacion *Usuarios = NULL;
  int tamanoInicial = 100; // Tamaño inicial del vector
  Usuarios =
      (struct Operacion *)calloc(tamanoInicial, sizeof(struct Operacion));
  ...
  for (int i = 1; i < num_registros; i++) {
    ...
    // ultimoUsuario); printf("Tiempo actual y anterior :%s y %s\n",
    ...
    if (strcmp(registros[i].IdUsuario, ultimoUsuario) == 0 &&
        enLaMismaHora(registros[i].FECHA_INICIO, ultimoTiempo) == 1 &&
        registros[i].flag == 0) {
      ...
      strcpy(Usuarios[num_usuario].IdTipoOperacion,
             registros[i - 1].IdTipoOperacion);
      ...
      num_usuario++;
               ...
               i == num_registros) {
      // Hemos pasado al siguiente usuario (o es el ultimo usuario), ya que el
      // vector Registros esta ordenado por usuario, e ignora aquellas
      // operaaciones con flag a 1 por lo que toca comprobar si el anterior
      // usuario ha hecho o no 5 o más operaciones en una hora
      int cumpleCondicion = 0; // Variable que me dirá si se han realizado 5
                               // operaciones en una hora o más.
      if (num_usuario > 4) {
        cumpleCondicion = 1; // Se cumple que haya hecho un mimo usuario 5
                             // operaciones o más en una hora.

        // Si la condicion si se cumple, entonces muestro las operaciones del
        // usuario.
        if (cumpleCondicion == 1) {
          printf(
              "\n\nESTE USUARIO HA HECHO 5 O MAS MOVIMIENTOS EN UNA HORA:\n");
          for (int j = 0; j < num_usuario; j++) {
            printf("IdUsuario: %s, FECHA_INICIO: %s", Usuarios[j].IdUsuario,
                   Usuarios[j].FECHA_INICIO);
          }
          cumpleCondicion = 0;
        }
      }
      // Libero el vector Usuarios dinámico para poder reutilizarlo en un
      // posible siguiente usuario.
      free(Usuarios);
      struct Operacion *Usuarios = NULL;
      int tamanoInicial = 100; // Tamaño inicial del vector
      Usuarios =
...
void *pattern2(void *arg) {
  ...
  for (int i = 1; i < num_registros; i++) {
    ...
    if (strcmp(registros[i].IdUsuario, ultimoUsuario) == 0 &&
      ...
      // printf("\nENTRO num_user: %d USER: %s, IMPORTE: %f ",num_usuario,
      // Usuarios[num_usuario].IdUsuario, Usuarios[num_usuario].Importe);
      num_usuario++;

    } else if (strcmp(registros[i].IdUsuario, ultimoUsuario) != 0) {
      // printf("\nNO ENTRO num_user: %d USER: %s, IMPORTE: %f ",num_usuario,
      // registros[i].IdUsuario, registros[i].Importe);
      ...
      if (num_usuario > 1) {
        ...
        if (cumpleCondicion == 1) {
          ...
          for (int j = 0; j < num_usuario; j++) {
            printf("\t\tIdOperacion: %s, FECHA_INICIO: %s, IdUsuario: %s, "
                   "Importe: %f,\n",
                   Usuarios[j].IdOperacion, Usuarios[j].FECHA_INICIO,
                   Usuarios[j].IdUsuario, Usuarios[j].Importe);
          }
          num_usuario = 0;
          cumpleCondicion = 0;
        }
      ...
      // Libero el vector Usuarios dinámico para poder reutilizarlo en un
      // posible siguiente usuario. printf("Reinicio el user: %s en el num_user:
      // %d con importe: %f\n", Usuarios[num_usuario-1].IdUsuario, num_usuario,
      // Usuarios[num_usuario-1].Importe);
      free(Usuarios);
      struct Operacion *Usuarios = NULL;
      int tamanoInicial = 100; // Tamaño inicial del vector
      Usuarios =
...
sem_t mutex3;
void *pattern3(void *arg) { // Inicialización del semáforo
  sem_init(&mutex3, 0, 1);
  ...
  for (int i = 1; i < num_registros; i++) {
    ...
    if (strcmp(registros[i].IdUsuario, ultimoUsuario) == 0 &&
      ...
      Usuarios[num_usuario].DineroRet = registros[i].DineroRet;
      Usuarios[num_usuario].flag = registros[i].flag;
      // printf("\nENTRO num_user: %d USER: %s, Estado: %s ",num_usuario,
      // Usuarios[num_usuario].IdUsuario, Usuarios[num_usuario].Estado);
      num_usuario++;

    } else if (strcmp(registros[i].IdUsuario, ultimoUsuario) != 0) {
      // Hemos pasado al siguiente usuario (o es el ultimo usuario), ya que el
      // vector Registros esta ordenado por usuario, e ignora aquellas
      // operaaciones con flag a 1 por lo que toca comprobar si son 3 o más
      // operaciones que cumplen el patron
      int cumpleCondicion = 0;
      // El siguiente bucle se usará para comprobar si las operaciones de error
      // sucedieron en el mismo día.
      int cont = 0;
      ...
      if (cont > 2) {
        cumpleCondicion = 1; // Se cumple que haya hecho un mimo usuario 3
                             // operaciones o más en un día con error.
        ...
        if (cumpleCondicion == 1) {
          printf("\n\nESTE USUARIO HA TENIDO 3 O MAS ERRORES EN UN DIA\n",
                 num_usuario);
          for (int j = 0; j < num_usuario; j++) {
            printf("\t\tEstado: %s, FECHA_INICIO: %s, IdUsuario: %s, Importe: "
                   "%f,\n",
                   Usuarios[j].Estado, Usuarios[j].FECHA_INICIO,
                   Usuarios[j].IdUsuario, Usuarios[j].Importe);
          }
          num_usuario = 0;
          cumpleCondicion = 0;
...
void *pattern4(void *arg) {
  ...
  int tamanoInicial =
      100; // Tamaño inicial del vector (puedes ajustarlo según tus necesidades)
  Usuarios =
      (struct Operacion *)malloc(tamanoInicial * sizeof(struct Operacion));
  ...
  for (int i = 1; i < num_registros; i++) {
    ...
    if (strcmp(registros[i].IdUsuario, ultimoUsuario) == 0 &&
      ...
      strcpy(Usuarios[num_usuarios].IdTipoOperacion,
             registros[i].IdTipoOperacion);
      ...
      num_usuarios++;
    } else if (strcmp(registros[i].IdUsuario, ultimoUsuario) !=
               0) { // Hemos pasado al siguiente usuario, ya que el vector
                    // Registros esta ordenado por usuario, e ignora aquellas
                    // operaaciones con flag a 1
      // por lo que toca comprobar si el anterior usuario ha hecho o no los 4
      // tipos de operaciones
      int cumpleCondicion[4]; // Vector que me dirá si se han realizado una o
                              // más operaciones de cada tipo.
      int Cumple = 1;         // Variable de control para comprobar que todas se
                              // cumplen. La doy por cierta
      ...
      // Si tras haber analizado todas las operaciones de este usuario hay algun
      // tipo de operacion que no haya sido realizada, Cumple será 0.
      for (int k = 0; k < 3; k++) {
        if (cumpleCondicion[k] != 1) {
          Cumple = 0;
          // Reestablezco el vector de comprobación
          cumpleCondicion[0] = cumpleCondicion[1] = cumpleCondicion[2] =
              cumpleCondicion[3] = 0;
        }
      }
      // Si la condicion si se cumple, entonces muestro las operaciones del
      ...
      if (Cumple == 1) {
        ...
        for (int j = 0; j < num_usuarios; j++) {
          printf("IdOperacion: %s, ID Usuario: %s, Fecha Inicio: %s\n",
                 Usuarios[j].IdOperacion, Usuarios[j].IdUsuario,
                 Usuarios[j].FECHA_INICIO);
        }
        // Ahora, sabiendo que se cumplió el patron, tengo que cambiar todos los
        // flags de este usuario a 1, para no volver a revisar este patron
        // posteriormente.
      ...
      // Libero y vacio el espacio de memoria, para volver a usarlo con el nuevo
      // usuario siguiente.
      free(Usuarios);   // Liberar la memoria asignada al vector de estructuras
                        // Usuarios
      Usuarios = NULL;  // Establecer el puntero a NULL para indicar que el
                        // vector está vacío
  ...
  for (int i = 1; i < num_registros; i++) {
    ...
    if (strcmp(registros[i].IdUsuario, ultimoUsuario) == 0 &&
      ...
      if (registros[i - 1].DineroRet > registros[i - 1].DineroIngr) {
        printf("\n\nEl usuario %s ha retirado más dinero del que ha "
               "ingresado.\n\n",
...
int readConsolidatedFile() {
  ...
  while (num_registros < SharedMemory_ptr->filesCount) {
    ...
    registros[num_registros].Sucursal =
        SharedMemory_ptr->files->sucursal_number;
    ...
    sscanf(SharedMemory_ptr->files[num_registros].line,
           "%[^;];%[^;];%[^;];%[^;];%[^;];%d;%f€;%[^;]",
...
void convertir_fecha(const char *fecha_str, struct tm *fecha_tm) {
  sscanf(fecha_str, "%2d/%2d/%4d%2d:%2d", &fecha_tm->tm_mday, &fecha_tm->tm_mon,
         &fecha_tm->tm_year, &fecha_tm->tm_hour, &fecha_tm->tm_min);
