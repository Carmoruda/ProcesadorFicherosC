      ...
      if (cumpleCondicion == true) {
        ...
        for (int i = 0; i < contadorOperaciones; i++) {
          printf("Sucursal: "
                 ...
                 registros[i].Estado);
          // Cambio el flag a 1 para evitar que se repita el patron
          // incesablemente.
          registros[i].flag = 1;
        }
...
void *pattern2(void *arg) {
  ...
  for (int i = 1; i < num_registros; i++) {
    ...
    if (strcmp(registros[i].IdUsuario, ultimoUsuario) == 0 &&
        ...
        registros[i].Importe < 0 && registros[i].flag == 0) {
...
void *pattern3(void *arg)
{
    ...
    for (int i = 1; i < num_registros; i++)
    {
        // Verificar si es la misma persona y si la operación está dentro del
rango
        // de una hora
            ...
            registros[i].flag == 0)
        {
            reg_patrones[contadorOperaciones] = registros[i];
            contadorOperaciones++;
            // Si el usuario realiza 3 o más operaciones de retiro en un mismo
día. if (contadorOperaciones >= 3)
            {
                cumpleCondicion = true;
            }
        }
        else
        {
            if (cumpleCondicion == true)
            {
                ...
                for (int i = 0; i < contadorOperaciones; i++)
                {
                    printf("Sucursal: "
                           "%d,IdOperacion: %s, FECHA_INICIO: %s, FECHA_FIN: %s,
" "IdUsuario: %s, IdTipoOperacion: %s, NoOperacion: %d, " "Importe: %.2f,
Estado: %s\n", registros[i].Sucursal, registros[i].IdOperacion,
                           registros[i].FECHA_INICIO, registros[i].FECHA_FIN,
                           registros[i].IdUsuario, registros[i].IdTipoOperacion,
                           registros[i].NoOperacion, registros[i].Importe,
                           registros[i].Estado);
                    // Cambio el flag a 1, para no revisar esta operacion más,
porque ya cumplió un patron registros[i].flag = 1;
                }
                cumpleCondicion = false;
            }

            // Reiniciar el contador de operaciones para un nuevo usuario o una
nueva
            // hora
            contadorOperaciones = 0;
        }
...
void *pattern4(void *arg)
{
    ...
    int tamanoInicial = 100;           // Tamaño inicial del vector (puedes
ajustarlo según tus necesidades) Usuarios = (struct Operacion
*)malloc(tamanoInicial * sizeof(struct Operacion));
...
registros[i].flag == 0)
        {
            ...
            strcpy(Usuarios[num_usuarios].IdOperacion, registros[i -
1].IdOperacion); strcpy(Usuarios[num_usuarios].FECHA_INICIO, registros[i -
1].FECHA_INICIO); strcpy(Usuarios[num_usuarios].FECHA_FIN, registros[i -
1].FECHA_FIN); strcpy(Usuarios[num_usuarios].IdUsuario, registros[i -
1].IdUsuario); strcpy(Usuarios[num_usuarios].IdTipoOperacion, registros[i -
1].IdTipoOperacion); Usuarios[num_usuarios].NoOperacion = registros[i -
1].NoOperacion; Usuarios[num_usuarios].Importe = registros[i - 1].Importe;
        ...
        {                                          // Hemos pasado al siguiente
usuario, ya que el vector Registros esta ordenado por usuario, e ignora aquellas
operaaciones con flag a 1
                                                   // por lo que toca comprobar
si el anterior usuario ha hecho o no los 4 tipos de operaciones int
cumpleCondicion[4] = {0, 0, 0, 0}; // Vector que me dirá si se han realizado una
o más operaciones de cada tipo. int Cumple = 1;                        //
Variable de control para comprobar que todas se cumplen. for (int j = 0; j <
num_usuarios; j++)
            {
                tipo = Usuarios[j].IdTipoOperacion[7]; // Tipo guarda el tipo de
operación, basándonos en la posicion numero 7 que es la diferencial. switch
(tipo)
                {
                case '1':
                    cumpleCondicion[0] = 1;
            ...
            // Si tras haber analizado todas las operaciones de este usuario hay
algun tipo de operacion que no haya sido realizada, Cumple será 0. for (int k =
0; k < 4; k++)
            {
            ...
            // Si la condicion si se cumple, entonces muestro las operaciones
del usuario. if (Cumple = 1)
            {
...
Estado: %s\n", Usuarios[j].Sucursal, registros[i].IdOperacion,
                ...
                // Ahora, sabiendo que se cumplió el patron, tengo que cambiar
todos los flags de este usuario a 1, para no volver a revisar este patron
posteriormente. for (int r = 0; r < num_registros; r++)
                {
            ...
            // Libero y vacio el espacio de memoria, para volver a usarlo con el
nuevo usuario siguiente. free(Usuarios);   // Liberar la memoria asignada al
vector de estructuras Usuarios Usuarios = NULL;  // Establecer el puntero a NULL
para indicar que el vector está vacío num_usuarios = 0; // Restablecer el
contador de usuarios Usuarios = (struct Operacion *)malloc(tamanoInicial *
sizeof(struct Operacion));
        }
