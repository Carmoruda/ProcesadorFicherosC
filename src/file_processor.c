...
/// @brief Leer la información del archivo de configuración
/// @param registros Vector de estructuras con todas las operaciones extraidas
/// del .csv
...
int superaLimiteOperaciones(struct Operacion *registros, int inicio, int fin) {
  ...
  for (int i = inicio + 1; i < fin; i++) {
    ...
    // Si la diferencia de tiempo es menor o igual a una hora, incrementar el
...
void patron1(void *arg) {
  ...
  while (i < num_registros) {
    ...
    while (j < num_registros && registros[j].IdUsuario == idUsuarioActual) {
      ...
      if (superaLimiteOperaciones(registros, i, j)) {
        ...
        printf("Usuario %d ha realizado más de 5 operaciones en menos de 1 "
               "hora:\n",
               idUsuarioActual);
        for (int k = i; k < j; k++) {
          printf("IdOperacion: %d, FECHA_INICIO: %s, FECHA_FIN: %s, IdUsuario: "
                 "%d, IdTipoOperacion: %d, NoOperacion: %d, Importe: %.2f, "
                 "Estado: %s\n",
                 registros[k].IdOperacion, registros[k].FECHA_INICIO,
                 registros[k].FECHA_FIN, registros[k].IdUsuario,
                 registros[k].IdTipoOperacion, registros[k].NoOperacion,
                 registros[k].Importe, registros[k].Estado);
        }
  ...
  for (int i = 0; i < num_registros; i++) {
    printf("IdOperacion: %d, FECHA_INICIO: %s, FECHA_FIN: %s, IdUsuario: %d, "
           "IdTipoOperacion: %d, NoOperacion: %d, Importe: %.2f, Estado: %s\n",
           registros[i].IdOperacion, registros[i].FECHA_INICIO,
           registros[i].FECHA_FIN, registros[i].IdUsuario,
           registros[i].IdTipoOperacion, registros[i].NoOperacion,
           registros[i].Importe, registros[i].Estado);
  }
