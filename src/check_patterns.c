...
void *pattern3(void *arg) {
  ...
  for (int i = 0; i < num_registros; i++) {
    ...
    if (strcmp(registros[i].IdUsuario, ultimoUsuario) &&
        enElMismoDía(registros[i].FECHA_INICIO, ultimoTiempo) == 1 &&
        strcmp(registros[i].Estado, "Error") && registros[i].flag == 0) {
      reg_patrones[contadorOperaciones] = registros[i];
      contadorOperaciones++;
      // Si el usuario realiza 3 o más operaciones de retiro en un mismo día.
      if (contadorOperaciones >= 3) {
      ...
      if (cumpleCondicion == true) {
        ...
        for (int i = 0; i < contadorOperaciones; i++) {
          printf("Sucursal: "
                 ...
                 registros[i].NoOperacion, registros[i].Importe,
                 registros[i].Estado);
          // Cambio el flag a 1, para no revisar esta operacion más, porque ya
          // cumplió un patron
          registros[i].flag = 1;
        }
        cumpleCondicion = false;
...
void *pattern4(void *arg) {
  ...
  for (int i = 1; i < num_registros; i++) {
    ...
    if (strcmp(registros[i].IdUsuario, ultimoUsuario) &&
      ...
      strcpy(Usuarios[num_usuarios].FECHA_INICIO,
             registros[i - 1].FECHA_INICIO);
      ...
      num_usuarios++;
    } else { // Hemos pasado al siguiente usuario, ya que el vector Registros
             // esta ordenado por usuario, e ignora aquellas operaaciones con
             // flag a 1
      // por lo que toca comprobar si el anterior usuario ha hecho o no los 4
      // tipos de operaciones
      int cumpleCondicion[4] = {0, 0, 0,
                                0}; // Vector que me dirá si se han realizado
                                    // una o más operaciones de cada tipo.
      int Cumple =
          1; // Variable de control para comprobar que todas se cumplen.
      for (int j = 0; j < num_usuarios; j++) {
        tipo =
            Usuarios[j].IdTipoOperacion[7]; // Tipo guarda el tipo de operación,
                                            // basándonos en la posicion numero
                                            // 7 que es la diferencial.
        switch (tipo) {
        case '1':
          cumpleCondicion[0] = 1;
        case '2':
