  ...
  for (int i = 0; i < num_registros; i++) {
    ...
    if (strcmp(registros[i].IdUsuario, ultimoUsuario) &&
      ...
      if (cumpleCondicion == true) {
        ...
        for (int i = 0; i < contadorOperaciones; i++) {
          printf("Sucursal: %d,IdOperacion: %s, FECHA_INICIO: %s, FECHA_FIN: "
                 "%s, IdUsuario: %s, IdTipoOperacion: %s, NoOperacion: %d, "
                 "Importe: %.2f, Estado: %s\n",
                 registros[i].Sucursal, registros[i].IdOperacion,
                 registros[i].FECHA_INICIO, registros[i].FECHA_FIN,
                 registros[i].IdUsuario, registros[i].IdTipoOperacion,
...
void *pattern4(void *arg) {
  ...
  for (int i = 0; i < num_registros; i++) {
    ...
    if (strcmp(registros[i].IdUsuario, ultimoUsuario) &&
      ...
      printf("Estoy\n\n");
      if (cumpleCondicion[0] == cumpleCondicion[1] == cumpleCondicion[2] ==
          cumpleCondicion[3] == 1) {
        printf("Datos de operacion que provoca el patron 3:\n");
        for (int i = 0; i < contadorOperaciones; i++) {
          printf("ucursal: %d,IdOperacion: %s, FECHA_INICIO: %s, FECHA_FIN: "
                 "%s, IdUsuario: %s, IdTipoOperacion: %s, NoOperacion: %d, "
                 "Importe: %.2f, Estado: %s\n",
                 registros[i].Sucursal, registros[i].IdOperacion,
                 registros[i].FECHA_INICIO, registros[i].FECHA_FIN,
